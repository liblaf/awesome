...
async def fetch_user_collection_paged(
    client: httpx.AsyncClient, user: str, offset: int = 0
) -> PagedUserCollection:
...
async def fetch_user_collection(user: str) -> list[UserSubjectCollection]:
    data: list[UserSubjectCollection] = []
    async with httpx.AsyncClient(headers={"User-Agent": USER_AGENT}) as client:
        while True:
            response_body: PagedUserCollection = await fetch_user_collection_paged(
                client, user, offset=len(data)
            )
